...
const initializeDb = async () => {
    ...
    try {
        ...
        `);
        const constraintExists = await client.query(`SELECT 1 FROM pg_constraint WHERE conname = 'daily_quest_status_pkey' AND conrelid = 'daily_quest_status'::regclass;`);
        if (constraintExists.rowCount === 0) {
            await client.query(`ALTER TABLE daily_quest_status ADD CONSTRAINT daily_quest_status_pkey PRIMARY KEY (player_id, quest_key);`);
        }
        ...
        if (parseInt(ptCountRes.rows[0].count, 10) === 0) {
            for (let i = 1; i <= 8; i++) await client.query('INSERT INTO perilous_trials (name) VALUES ($1) ON CONFLICT (name) DO NOTHING', [`PT${i}`]);
        }
        ...
        if (parseInt(ccCountRes.rows[0].count, 10) === 0) {
            ...
            for (const timer of defaultTimers) await client.query('INSERT INTO class_change_timers (label, weeks_after_start) VALUES ($1, $2) ON CONFLICT DO NOTHING', [timer.label, timer.weeks]);
        }
