...
router.get('/', async (req, res) => {
    try {
        ...
        const serverOffsetHours = -4; const now = new Date(); const serverTime = new Date(now.valueOf() + (serverOffsetHours * 60 * 60 * 1000)); const serverDay = serverTime.getUTCDay();
        const getNextReset = (targetDay) => { const reset = new Date(serverTime); reset.setUTCHours(5, 0, 0, 0); if (targetDay !== undefined) { const daysUntilTarget = (targetDay - serverDay + 7) % 7; reset.setUTCDate(reset.getUTCDate() + daysUntilTarget); } if (reset < serverTime) { reset.setUTCDate(reset.getUTCDate() + (targetDay === undefined ? 1 : 7)); } return reset; };
